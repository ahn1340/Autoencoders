...
class AutoEncoder(nn.Module):
    def __init__(self, hidden_dim=16):  #TODO: add arguments for later
        super(AutoEncoder, self).__init__()
        self.encoder = nn.Sequential(
            ReLUConvBN(C_in=3, C_out=32, kernel_size=(4, 4), stride=2, padding=1, dilation=1, affine=True),  # 16
            ReLUConvBN(C_in=32, C_out=64, kernel_size=(4, 4), stride=2, padding=1, dilation=1, affine=True),  # 8
            ReLUConvBN(C_in=64, C_out=128, kernel_size=(4, 4), stride=2, padding=1, dilation=1, affine=True),  # 4
            ReLUConvBN(C_in=128, C_out=256, kernel_size=(4, 4), stride=2, padding=1, dilation=1, affine=True),  # 2
            nn.Flatten(),
